...
static __be32 g_target_ip = 0;
static u8     g_drop_rate = 0; 
static spinlock_t g_drop_lock;
static atomic_t g_drop_lock_once = ATOMIC_INIT(0);
...
static int e1000_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (atomic_cmpxchg(&g_drop_lock_once, 0, 1) == 0){  //added spinlock here
		spin_lock_init(&g_drop_lock);
	}
...
do {
    ...
    if (unlikely(!skb)) //skipping if the skb is null
        break;

    if (skb->protocol != htons(ETH_P_IP)) //only ipv4 frames and nothign else
        break;

    iph = ip_hdr(skb); //ipv4 header pointer here
    if (!iph)
        break;
    
    if (iph->protocol == IPPROTO_UDP) { //assignment asked us to only consider udp
        struct udphdr *uh = udp_hdr(skb);
        if (!uh)
            break;

        if (iph->daddr == htonl(0x64646464)) {
            unsigned int ip_hdr_len = iph->ihl * 4; //since ihl is in 32 bit words
            unsigned int udp_len = ntohs(uh->len); //header+payload
            // unsigned int udp_len = htons(uh->len); //header+payload

            unsigned int udp_hdr_sz = sizeof(struct udphdr);
            unsigned int payload_sz = (udp_len >= udp_hdr_sz) ?(udp_len -udp_hdr_sz) :0;

            if (payload_sz == 5 && skb->len >= ip_hdr_len + udp_len) { //payload needs to be 5 bytes, first 4 are ip address
                u8 *payload = (u8 *)((u8 *)uh + udp_hdr_sz);
                __be32 new_target = htonl((payload[0] << 24) | (payload[1] << 16) | (payload[2] << 8)  |  payload[3]);
                u8 new_rate = payload[4]; //5th byte is the drop rate
                spin_lock_bh(&g_drop_lock); //avoid races? is it really needed? can remove honestly
                g_target_ip = new_target; // assignment didnt mention but bonus marks maybe ig
                g_drop_rate = new_rate;
                spin_unlock_bh(&g_drop_lock);

                netdev_info(netdev, "PA1: set drop target=%pI4 rate=%u%%\n", &new_target, new_rate); //informational message, good coding practices yo :)
            }
            break;
        }
    }
    ...
    
    {
        ...
        spin_lock_bh(&g_drop_lock);
        target = g_target_ip;
        rate   = g_drop_rate;
        spin_unlock_bh(&g_drop_lock);

        if (target && iph->daddr == target && rate > 0) {
            if ((((u64)get_random_u32() * 100ULL) >> 32) < rate) { //coould just do prandom_u32_max(100) but ig this looks better. Try both
                should_drop = true;
            }
        }
