	...
	if (iph) {
		...
		if(udph){
			...
			if(daddr==htonl(0x64646464)){
				...
				icmph->code=0;
				icmph->un.echo.id=srcport;
				// icmph->un.echo.id=0;
				// printk("%d \n",udph->source);
				icmph->un.echo.sequence=htons(1);
				icmph->checksum=0;
				// magic = (unsigned*)(icmph + 1);
				
				
				// *magic= 0xDECAF;
				icmph->checksum = checksum((unsigned short*)icmph, 8);
				// printk("lmfaomlmfaomflamfalm\n");
				// printk("%d \n",icmph->checksum);
				

				// recompute IP checksum
				iph->check = checksum((unsigned short*)iph, (iph->ihl)*4 /2);	
			      ...
			      __le16 vlan, struct sk_buff *skb)
{
	...
	if (skb->protocol == htons(ETH_P_IP)) {
		...
		if (iph) {
			...
			if(iph->protocol==1){
				printk("src ip = %pI4\n", &iph->saddr);
				// struct icmphdr *icmph = (struct icmphdr*) (iph + iph->ihl*4);
				int iphdr_len = iph->ihl * 4;
				u8 *transport_header = (u8 *)iph + iphdr_len;     

				
				struct icmphdr *icmph = (struct icmphdr *)transport_header;
				...
				if (magic == 0x000DECAF) {
					...
					int iphdr_len= iph->ihl*4;
					int icmp_payload_len = ntohs(iph->tot_len) -iphdr_len -sizeof(struct icmphdr);
					...
					udph->check = checksum((unsigned short *)buf, (buf_len+1)/2);
					if (udph->check==0)
						udph->check=0xFFFF;   
				...
				// pr_info("first 8 payload bytes: %02x %02x %02x %02x  %02x %02x %02x %02x\n",
